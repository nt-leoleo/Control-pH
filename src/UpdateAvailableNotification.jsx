...
const UpdateAvailableNotification = ({ initialUpdateInfo = null }) => {
  ...
  const handleUpdate = async () => {
    if (!updateInfo?.zipUrl) {
      console.error('No OTA ZIP URL available');
      alert('No hay paquete ZIP disponible para OTA. Descarga e instala el APK manualmente.');
      return;
    }
    ...
    try {
      await UpdatingService.downloadAndInstall(updateInfo, (progress) => setDownloadProgress(progress));
    } catch (error) {
      console.error('Error installing update:', error);
      setIsDownloading(false);
      setDownloadProgress(0);
      alert(`Error descargando actualizacion: ${error.message}`);
    }
  };
      ...
      <div className="update-notification-card">
        <div className="update-notification-header">
          ...
          <button
            ...
            disabled={isDownloading || updateInfo?.mandatory}
            aria-label="Cerrar"
          >
            x
          </button>
        ...
        <div className="update-notification-content">
          <p className="update-version">
            Nueva version: <strong>v{updateInfo?.version}</strong>
          </p>
          <p className="update-changelog">
            {updateInfo?.changelog || 'Nueva version disponible con mejoras y correcciones'}
          </p>
          ...
          {isDownloading && (
            <div className="download-progress">
              ...
              <p className="progress-text">Descargando actualizacion...</p>
            </div>
          )}
        </div>
        ...
        <div className="update-notification-note">
          <small>
            La app se reiniciara automaticamente despues de descargar la actualizacion.
            Tus datos se guardaran en Firebase.
          </small>
        </div>
      </div>
    </div>
